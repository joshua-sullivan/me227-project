...
function [delta_rad, Fx_N] = gerdes_me227_controller(s_m, e_m, deltaPsi_rad, Ux_mps, Uy_mps, r_radps, modeSelector, path)
    ...
    % Runs the lateral PD controller with a feedforward/PI-feedback longitudinal controller.
        ...
        Kp_lat = .4;
        Kd_lat = 0.125; %0.9
        e_dot = state.Ux_mps*sin(state.deltaPsi_rad)+state.Uy_mps*cos(state.deltaPsi_rad);
        delta_rad = -Kp_lat * (state.e_m + Kd_lat * e_dot);
        if delta_rad > 20*pi/180
            delta_rad = 20*pi/180;
        elseif delta_rad < -1*20*pi/180
            delta_rad = -1*20*pi/180;
        end
